...
export function renderLogin(container, onLoginSuccess) {
  let currentUserFound = null;

  container.innerHTML = `
    <div class="login-page-bg">
      <div class="login-bg-decor top-right"></div>
      <div class="login-bg-decor bottom-left"></div>

      <div class="login-card" id="loginCard">
        <!-- SVG Interactive Yeti Avatar from ng-animated-login -->
          ...
          <div>
            <svg class="mySVG" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200">
              ...
              <g class="body">
                ...
                <path fill="none" stroke="#3A5E77" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" d="M193.3,135.9 c-5.8-8.4-15.5-13.9-26.5-13.9H151V72c0-27.6-22.4-50-50-50S51,44.4,51,72v50H32.1c-10.6,0-20,5.1-25.8,13"/>
        ...
        <h1 class="login-card-title">IRMS Portal</h1>
        <p class="login-card-subtitle">Inventory Recovery Management System</p>
        ...
        <form id="loginForm" autocomplete="off" onsubmit="return false;">
          ...
          <!-- Staff Name Auto-filled Input Group (Smooth Expandable Container) -->
          <div class="inputGroup expandable-inputGroup" id="staffNameGroup">
            <label for="staffNameInput">Staff Name</label>
            <input type="text" id="staffNameInput" readonly placeholder="Auto-filled Staff Name" />
          </div>
  ...
  // DOM Elements
  const staffIdInput = container.querySelector('#staffIdInput');
  const staffNameGroup = container.querySelector('#staffNameGroup');
  const staffNameInput = container.querySelector('#staffNameInput');
  const staffIdFeedback = container.querySelector('#staffIdFeedback');
  const passwordInput = container.querySelector('#passwordInput');
  const passwordFeedback = container.querySelector('#passwordFeedback');
  const loginForm = container.querySelector('#loginForm');
  const loginCard = container.querySelector('#loginCard');
  const submitBtn = container.querySelector('#submitBtn');

  // SVG Elements for GSAP animation
  ...
  function coverEyes() {
    gsap.to(armL, { x: -93, y: 2, rotation: 0, duration: 0.45, ease: Quad.easeOut });
    gsap.to(armR, { x: -93, y: 2, rotation: 0, duration: 0.45, ease: Quad.easeOut, delay: 0.1 });
  }

  function uncoverEyes() {
    gsap.to(armL, { y: 220, rotation: 105, duration: 1.35, ease: Quad.easeOut });
    gsap.to(armR, { y: 220, rotation: -105, duration: 1.35, ease: Quad.easeOut, delay: 0.1 });
  }

  function resetFace() {
    gsap.to([eyeL, eyeR], { x: 0, y: 0, duration: 1, ease: Expo.easeOut });
    gsap.to(nose, { x: 0, y: 0, scaleX: 1, scaleY: 1, duration: 1, ease: Expo.easeOut });
    gsap.to(mouth, { x: 0, y: 0, rotation: 0, duration: 1, ease: Expo.easeOut });
    gsap.to(chin, { x: 0, y: 0, scaleY: 1, duration: 1, ease: Expo.easeOut });
    gsap.to([face, eyebrow], { x: 0, y: 0, skewX: 0, duration: 1, ease: Expo.easeOut });
    gsap.to([outerEarL, outerEarR, earHairL, earHairR, hair], { x: 0, y: 0, scaleY: 1, duration: 1, ease: Expo.easeOut });
  }

  function updateFaceTrack() {
    const valLength = staffIdInput.value.length;
    const maxLen = 15;
    const ratio = Math.min(valLength / maxLen, 1);
    const offsetX = (ratio - 0.5) * eyeMaxHorizD * 2;
    const offsetY = ratio * 4;

    gsap.to([eyeL, eyeR], { x: offsetX, y: offsetY, duration: 0.5, ease: Power2.easeOut });
    gsap.to(nose, { x: offsetX * 0.8, y: offsetY, duration: 0.5, ease: Power2.easeOut });
    gsap.to(mouth, { x: offsetX * 0.7, y: offsetY, duration: 0.5, ease: Power2.easeOut });
    gsap.to([face, eyebrow], { x: offsetX * 0.4, skewX: offsetX * 0.3, duration: 0.5, ease: Power2.easeOut });
  }

  // Event Listeners for Yeti animation
  staffIdInput.addEventListener('focus', updateFaceTrack);
  staffIdInput.addEventListener('input', () => {
    updateFaceTrack();
    validateStaffId();
  });
  staffIdInput.addEventListener('blur', () => {
    resetFace();
    const isValid = validateStaffId();
    if (staffIdInput.value.trim() && !isValid) {
  ...
  // Staff ID Validation Logic
  function validateStaffId() {
    const val = staffIdInput.value.trim();
    if (!val) {
      staffIdFeedback.innerHTML = '';
      staffNameGroup.classList.remove('expanded');
      staffNameInput.value = '';
      currentUserFound = null;
      return false;
    }

    const user = db.lookupStaffId(val);
    if (user) {
      currentUserFound = user;
      staffNameGroup.classList.add('expanded');
      staffNameInput.value = user.name;
      staffIdFeedback.innerHTML = `
        <div class="autofill-badge success">
          <span class="material-icons-round" style="font-size: 16px;">check_circle</span>
          <span>Staff found: <strong>${user.name}</strong> (${user.role})</span>
        </div>
      `;
      loginCard.classList.remove('shake');
      return true;
  ...
  loginForm.addEventListener('submit', (e) => {
    ...
    const isValidStaff = validateStaffId();
    if (!isValidStaff) {
      loginCard.classList.remove('shake');
      void loginCard.offsetWidth;
      loginCard.classList.add('shake');
    ...
    setTimeout(() => {
      onLoginSuccess(currentUserFound);
    }, 800);
  });
}
